...
- __ifdown__ seguido do nome da interface permite desativá-la;
- interfaces podem ser habilitadas para 'dhcp' ou 'static' no campo 'inet'; caso 'static', seguem-se o IP após 'address' em uma linha e o IP iniciado por 'gateway' em outra linha;
...
### /etc/nsswitch.conf
- arquivo que define fontes de correspondências nome-IP;

### /etc/hosts
- arquivos com banco de dados de hosts e IPs;

### /etc/resolv.conf
- arquivo que contém as diretrizes para o método 'resolver', contendo entradas como 'nameserver', para definir o servidor DNS entrando seu IP em seguida, 'domain', para definir o domínio local,  e 'search', para uma lista de domínios;
...
- __categorias ou objetos do nmcli:__
	...
	- device: dispositivos gerenciados;
		...
		- ```$ nmcli device wifi connect SSID_NAME```: conecta à rede abrindo umacaixa de diálogo no modo gráfico para a inserção da senha; em modo texto exclusivamente, a senha pode ser inserida na mesma linha do comando, com a sintaxe: ```$ nmcli device wifi connect SSID_NAME password PASSWORD```;
