...
- ```*.=debug;\
		auth,authpriv.none;\
	news.none;mail.none		-/var/log/debug```: as mensagens 'debug' e nenhuma outra '=' serão gravadas em '/var/log/debug', exceto se vierem  dos recursos 'auth,authpriv', 'news' e 'mail'.
...
- '__/etc/logrotate.conf__' é o arquivo de configuração, cujos comentários apresentam explicações das finalidades das opções globais, entre as quais costa a inclusão dos arquivos de configuração extras em: - '__/etc/logrotate.d__' é o diretório onde se incluem arquivos extras de configuração com as linhas:
	...
	- 'endrotate' infica o final do script postrotate.
...
- '__/etc/systemd/journal.conf__' é o seu arquivo de configuração junto com arquivos '*.conf' em 'journal.conf.d/':
...
### journalctl
- utilitário que exibe os registros de log pelo 'systemd-journald', os quais são armazenados em diários binários;
- __opções:__
