...
- ecdsa: 'Elliptic Curve Digital Signature Algorithm' é um parimoramento do DSA, com três tamanhos possíveis de curvas elípticas em bits: 256, 384 ou 521;
...
- __opções:__
	...
	- '-l -f' listam as impressões digitais das chaves, passando o caminho do arquivo da chave após '-f';
...
- '__ssh_config__' é o arquivo de configuração do cliente e '__sshd_config__', do servidor;
...
- ```$ gpg --list-keys ``` exibe o conteúdo do chaveiro público;
- ```$ gpg --export USER_ID > FILE.pub.key ``` permite exportar a chave pública;
	-  acrescentando '--armor' ou '-a' a 'gpg --export', cria-se uma saída ASCII blindada;
#### Key Sharing
